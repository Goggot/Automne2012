...
public class Echiquier
{
  ...
  public void debuter()
  {
    ...
    public boolean cheminPossible(Position depart, Position arrive)    // D�FINITION DES D�PLACEMENTS POSSIBLE EN TOUT, PAS PAR PI�CE
    {
        boolean valid = true;
        ...
        if (depart.getLigne() - arriveLigne > 0 && depart.getColonne() == arrive.getColonne())                      // V�rification du chemin vers le haut sur les lignes
        {
        ...
        if (depart.getLigne() - arriveLigne < 0 && depart.getColonne() == arrive.getColonne())                      // V�rification du chemin vers le bas sur les lignes
        {
            for (int i=depart.getLigne()+1; i<depart.getLigne(); i++)
                if (location[i][arrive.getColonne()].estOccupe())
                    valid = false;
        ...
        if (depart.getColonne() - arriveColonne > 0 && depart.getLigne() == arrive.getLigne())                  // V�rification du chemin vers le haut sur les colonnes
        {
        ...
        if (depart.getColonne() - arriveColonne < 0 && depart.getLigne() == arrive.getLigne())                  // V�rification du chemin vers le bas sur les colonnes
        {
            for (int i=depart.getColonne()+1; i<arrive.getColonne(); i++)
        ...
        if (depart.getColonne() - arrive.getColonne() > 0 && depart.getLigne() != arrive.getLigne())                  // V�rification de la diagonale gauche
        {
            if (depart.getLigne() - arriveLigne > 0)                      // V�rification du chemin vers le haut
            {
            ...
            else if (depart.getLigne() - arriveLigne < 0)                 // V�rification du chemin vers le bas
            {
                int ligne = depart.getLigne();
                for (int i=depart.getColonne()+1; i<arrive.getColonne(); i++)
                    if (location[ligne++][i].estOccupe())
        ...
        if (depart.getColonne() - arrive.getColonne() < 0 && depart.getLigne() != arrive.getLigne())                  // V�rification de la diagonale droite
        {
            if (depart.getLigne() - arriveLigne > 0)                      // V�rification du chemin vers le haut
            {
                int ligne = depart.getLigne();
    ...
    public boolean captureParUnPionPossible(Position depart, Position arrive)
    {
        if (depart.getLigne() - arrive.getLigne() < 0 && location[depart.getLigne()][depart.getColonne()].getPiece().getCouleur() == "blanc")
          ...
          if (location[arrive.getLigne()][arrive.getColonne()].estOccupe())       // Si la case d'arriv�e est occup�
            if (location[depart.getLigne()][depart.getColonne()].getPiece().getCouleur() != 
                  location[arrive.getLigne()][arrive.getColonne()].getPiece().getCouleur()) // Si la piece pr�sente sur la case d'arriv�e est de couleur diff�rente
                return true;
            else
                return false;
          else
